...
from lakehouse.silver_maintain import (
    DATASET_ID,
    JOB,
    collect_snapshot,
...
def test_spec_snapshot_chains_expire_then_compact() -> None:
    as_of = date(2026, 9, 1)
    snap = spec_snapshot(as_of=as_of, retention_days=60, max_objects=8)
...
def test_collect_and_describe() -> None:
    ...
    assert snap["order"] == ["expire", "compact"]
    result = describe_silver_maintain(as_of=date(2026, 9, 1))
...
def test_cli_silver_maintain(capsys: object) -> None:
    code = main(["silver-maintain"])
    captured = capsys.readouterr()  # type: ignore[attr-defined]
    assert '"job": "silver.maintain"' in captured.out
