...
def test_spec_snapshot_chains_expire_then_compact() -> None:
    ...
    snap = spec_snapshot(as_of=as_of, retention_days=30, max_objects=2)
...
def test_collect_and_describe() -> None:
    ...
    assert snap["order"] == ["expire", "compact"]
    result = describe_gold_quarantine_maintain(as_of=datetime(2026, 9, 1, tzinfo=UTC))
...
def test_cli_gold_quarantine_maintain(capsys: object) -> None:
    code = main(["gold-quarantine-maintain"])
    captured = capsys.readouterr()  # type: ignore[attr-defined]
    assert '"job": "gold.quarantine.maintain"' in captured.out
