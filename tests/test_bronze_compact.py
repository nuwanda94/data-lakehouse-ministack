...
def test_classify_keep_and_compact() -> None:
    ...
    assert compact["target"] == compact_key(dt="2026-08-25")


def test_plan_counts() -> None:
    plan = plan_compact(
        [
            {"dt": "2026-09-01", "objects": 1},
            {"dt": "2026-08-25", "objects": 12},
        ],
        max_objects=8,
    )
...
def test_collect_and_describe() -> None:
    ...
    result = describe_bronze_compact()
...
def test_resolve_env_override(monkeypatch: object) -> None:
    monkeypatch.setenv("LAKEHOUSE_BRONZE_COMPACT_MAX_OBJECTS", "5")  # type: ignore[attr-defined]
...
def test_merge_event_payloads() -> None:
    merged = merge_event_payloads(
        [
            ...
            {"event_id": "b", "dt": "2026-09-01", "event_type": "purchase"},
            {"dt": "2026-09-01", "events": [{"event_id": "c"}]},
        ]
    )
...
def test_cli_bronze_compact(capsys: object) -> None:
    code = main(["bronze-compact"])
    captured = capsys.readouterr()  # type: ignore[attr-defined]
    assert '"dataset": "bronze.raw_events"' in captured.out
