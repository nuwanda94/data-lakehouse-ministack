...
def test_spec_snapshot_includes_expired_fixture() -> None:
    ...
    assert snap["expire"][0]["dt"] == "2026-07-28"
...
def test_resolve_env_override(monkeypatch: object) -> None:
    monkeypatch.setenv("LAKEHOUSE_BRONZE_RETENTION_DAYS", "14")  # type: ignore[attr-defined]
    assert resolve_retention_days() == 14
    assert resolve_retention_days(7) == 7


def test_cli_bronze_retention(capsys: object) -> None:
    code = main(["bronze-retention"])
...
def test_age_boundary_is_keep() -> None:
    as_of = date(2026, 9, 1)
    row = classify_partition(dt="2026-08-02", as_of=as_of, retention_days=30)
