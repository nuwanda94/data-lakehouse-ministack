"""Silver compact-after-retention (scheduled expire then compact).

MiniStack CI is hermetic, so this module always evaluates a spec snapshot
that chains Silver partition expiry and Silver object compact, then optionally
folds in live MiniStack results.

Operators run silver-retention and silver-compact as separate commands.
``silver-maintain`` is the scheduled path: expire partitions older than the
retention window, then compact remaining fragmented ``event_type=/dt=``
prefixes.

``python -m lakehouse silver-maintain`` prints JSON. Default is dry-run
...
from lakehouse.silver_compact import spec_snapshot as compact_spec
from lakehouse.silver_retention import (
    DEFAULT_RETENTION_DAYS,
    describe_silver_retention,
    resolve_retention_days,
)
from lakehouse.silver_retention import spec_snapshot as retention_spec
...
def collect_snapshot(
    *,
    ...
    """Run Silver retention first, then compact. Live when MiniStack answers."""
    ...
    expire = describe_silver_retention(
        retention_days=retention_days,
        as_of=as_of,
        apply=apply,
    )
    compact = describe_silver_compact(
        max_objects=max_objects,
        apply=apply,
    )
