...
(Bronze Hive ``events/dt=YYYY-MM-DD`` keys vs a configurable retention
window) and optionally folds in live S3 objects when MiniStack answers.

``python -m lakehouse bronze-retention`` prints JSON. Default is dry-run
(``apply=false``). Exit code 1 means expired partitions exist and
``--apply`` was requested but a delete failed.

Retention days come from ``LAKEHOUSE_BRONZE_RETENTION_DAYS`` (default 30)
or ``--retention-days``.
"""
...
def resolve_retention_days(explicit: int | None = None) -> int:
    if explicit is not None:
        return int(explicit)
    raw = os.environ.get("LAKEHOUSE_BRONZE_RETENTION_DAYS")
...
def classify_partition(
    *,
    ...
    """Mark one Bronze ``dt=`` partition as keep or expire."""
...
def plan_retention(
    ...
    *,
    ...
    """Classify a list of ``{dt, key?, objects?}`` Bronze partitions."""
    ...
    fixtures = [
        {
            "dt": today.isoformat(),
            "objects": 12,
            "key": f"events/dt={today.isoformat()}/evt-keep-today.json",
        },
        {
            "dt": (today - timedelta(days=7)).isoformat(),
            "objects": 8,
            "key": (f"events/dt={(today - timedelta(days=7)).isoformat()}/evt-keep-week.json"),
        },
        {
            "dt": (today - timedelta(days=budget + 5)).isoformat(),
            "objects": 3,
            "key": (
                f"events/dt={(today - timedelta(days=budget + 5)).isoformat()}/evt-expire-old.json"
            ),
        },
    ]
...
def _delete_expired(
    ...
    try:
        ...
        for row in expire:
            ...
            try:
                keys = list_keys(s3, settings.bronze_bucket, folder)
                for key in keys:
