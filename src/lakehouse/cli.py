...
def _cmd_outputs(args: argparse.Namespace) -> int:
    ...
    tf_dir = getattr(args, "tf_dir", None)
...
def _cmd_athena(args: argparse.Namespace) -> int:
    from lakehouse.athena import named_queries, register_athena

    if args.name:
        query = get_or_run(args.name)
        return _ok(query)
...
def get_or_run(name: str) -> dict:
    from lakehouse.athena import get_named_query, run_named_query

    try:
        live = run_named_query(name)
        if isinstance(live, dict) and live:
            live.setdefault("ok", True)
            return live
    except Exception:
        pass
    q = get_named_query(name)
    return {"ok": True, "name": q.name, q.name: q.sql, "sql": q.sql}
