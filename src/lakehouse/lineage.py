...
(default 0.80), the quality-split aggregate share against
``LAKEHOUSE_LINEAGE_QUALITY_AGGREGATE_FLOOR`` (default 0.15), and the
quality-split reject share against
``LAKEHOUSE_LINEAGE_QUALITY_REJECT_CEILING`` (default 0.50). The CLI
exits 1 when any floor is missed or the reject ceiling is exceeded.
...
DEFAULT_QUALITY_REJECT_CEILING = 0.50
QUALITY_REJECT_CEILING_ENV = "LAKEHOUSE_LINEAGE_QUALITY_REJECT_CEILING"
...
def resolve_quality_reject_ceiling(explicit: float | None = None) -> float:
    """Quality-split reject-share ceiling (0–1)."""

    if explicit is not None:
        return float(explicit)
    raw = os.environ.get(QUALITY_REJECT_CEILING_ENV)
    if raw and raw.strip():
        return float(raw)
    return DEFAULT_QUALITY_REJECT_CEILING
...
def path_ratio_alert(
    ...
    *,
    ...
    """Alert when family, Bronze-split, or quality-split cuts fail.

    Spec fixtures sit at 0.6667 family cleanse, 0.8571 Bronze-split
    cleanse, 0.1667 quality-split aggregate, and 0.3333 quality-split
    reject, so the defaults (0.60 / 0.80 / 0.15 floor / 0.50 ceiling)
    are green. Operators raise
    ...
    ``--quality-aggregate-floor``, or lower
    ``LAKEHOUSE_LINEAGE_QUALITY_REJECT_CEILING`` /
    ``--quality-reject-ceiling`` when a pipeline should fail the
    ...
    quality_ceiling = resolve_quality_reject_ceiling(quality_reject_ceiling)
    ...
    try:
        quality_reject = float(quality.get("reject") or 0.0)
    except (TypeError, ValueError):
        quality_reject = 0.0
    quality_total = int((ratios.get("quality_split") or {}).get("total") or 0)
    quality_floor_ok = quality_total <= 0 or quality_aggregate >= quality_floor
    quality_ceiling_ok = quality_total <= 0 or quality_reject <= quality_ceiling
    quality_ok = quality_floor_ok and quality_ceiling_ok
