...
scheduled path that walks every medallion zone plus both quarantine side
paths:
...
5. Gold quarantine expire-then-compact
...
step would delete. Silver quarantine runs after Silver; Gold quarantine
runs after Gold so cleaned metrics are maintained before the rejected
aggregate side path.
...
from lakehouse.gold_quarantine_maintain import (
    collect_snapshot as gold_quarantine_collect,
)
from lakehouse.gold_quarantine_maintain import spec_snapshot as gold_quarantine_spec
...
ZONES = ("bronze", "silver", "quarantine", "gold", "gold_quarantine")
ORDER = [
    "bronze.maintain",
    "silver.maintain",
    "quarantine.maintain",
    "gold.maintain",
    "gold.quarantine.maintain",
]
...
def spec_snapshot(*, as_of: date | None = None) -> dict[str, Any]:
    ...
    gold_quarantine = gold_quarantine_spec(as_of=today)
    zones = {
        "bronze": bronze,
        "silver": silver,
        "quarantine": quarantine,
        "gold": gold,
        "gold_quarantine": gold_quarantine,
    }
    return {
        "backend": "spec",
        "job": JOB,
...
def collect_snapshot(
    *,
    ...
    """Run Bronze, Silver, Quarantine, Gold, then Gold quarantine maintain."""
    ...
    gold_quarantine = gold_quarantine_collect(as_of=as_of, apply=apply)
    parts = (bronze, silver, quarantine, gold, gold_quarantine)
    ...
    if not gold_quarantine.get("ok"):
        errors.append("gold quarantine maintain failed")
    return {
        ...
        "expire_count": (
            ...
            + int(gold_quarantine.get("expire_count") or 0)
        ),
        "compact_count": (
            ...
            + int(gold_quarantine.get("compact_count") or 0)
        ),
        "deleted_count": (
            ...
            + int(gold_quarantine.get("deleted_count") or 0)
        ),
        "rewritten_count": (
            ...
            + int(gold_quarantine.get("rewritten_count") or 0)
        ),
        "bronze": bronze,
        "silver": silver,
        "quarantine": quarantine,
...
def describe_platform_maintain(
    *,
    ...
    gold_quarantine = snap.get("gold_quarantine") or {}
    return {
        "ok": bool(snap.get("ok")),
        ...
        "gold_quarantine_job": gold_quarantine.get("job"),
    }
